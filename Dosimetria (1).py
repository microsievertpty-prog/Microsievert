...
def ninox_fetch_records_lazy(team_id: str, db_id: str, table_id: str, per_page: int = 1000, timeout_s: int = 8):
    """Versión NO cacheada con timeout corto para no colgar la UI."""
...
def normalize_df(records):
    ...
    for r in records:
        f = r.get("fields", {}) or {}
        cod_rep = f.get("CÓDIGO DE DOSÍMETRO")
        if cod_rep is None: cod_rep = f.get("CÓDIGO_DOSÍMETRO")
        rows.append({
            "_id": r.get("id"),
...
def leer_dosis(upload):
    ...
    if 'dosimeter' not in df.columns:
        for alt in ['dosimetro','codigo','codigodosimetro','codigo_dosimetro','dosímetro']:
    ...
    for cand in ['hp007dosecorr','hp007dose','hp007','hp007corr']:
...
def _find_codigo_field(cols) -> Optional[str]:
    targets = {
        ...
        "CODIGO DE DOSIMETRO","CODIGO_DOSIMETRO"
    }
    for c in cols:
...
def build_formatted_excel(df_final: pd.DataFrame, header_lines: List[str], logo_bytes: Optional[bytes]) -> bytes:
    ...
    for r in range(1, 4+1): ws.row_dimensions[r].height = 20
    ...
    # Sección informativa (idéntica a versiones previas; omitido detalle por brevedad)
    row = last_row + 2
    ...
    # … (puedes mantener la sección extendida si la necesitas completa)
...
with tab1:
    ...
    # --- Conexión perezosa (lazy) a Ninox ---
    st.markdown("#### Conexión a Ninox (LISTA DE CÓDIGO)")
    col_a, col_b = st.columns([1,1])
    with col_a:
        do_connect = st.button("🔌 Conectar a Ninox ahora", use_container_width=True, key="tab1_connect")
    with col_b:
        short_timeout = st.number_input("Timeout (seg)", min_value=3, max_value=30, value=8, step=1, key="tab1_timeout")

    if do_connect:
        with st.spinner("Conectando a Ninox…"):
            try:
                df_participantes = ninox_fetch_records_lazy(TEAM_ID, DATABASE_ID, base_table_id, timeout_s=int(short_timeout))
                if df_participantes.empty:
                    st.warning("Conectado, pero LISTA DE CÓDIGO no tiene filas.")
                else:
                ...
                st.error("⏱️ Ninox tardó demasiado en responder. Intenta de nuevo o ajusta el timeout.")
    ...
    if st.button("Subir TODO a Ninox (tabla REPORTE)", key="tab1_btn_upload"):
        ...
        if df_final is None or df_final.empty:
            ...
            if res.get("ok"):
                ...
                try:
                    df_check = ninox_fetch_records_lazy(TEAM_ID, DATABASE_ID, report_table_id, timeout_s=8)
...
with tab2:
    ...
    with st.sidebar:
        ...
        files = st.file_uploader("Archivos de dosis (filtrar por CÓDIGO DE DOSÍMETRO) — Opcional", type=["csv","xlsx","xls"], accept_multiple_files=True, key="tab2_codes")
    ...
    if df_company_type.empty:
        st.warning("No hay registros que cumplan los filtros de Compañía y/o Tipo.")
        st.stop()
    ...
    if df_all_for_people.empty:
        st.warning("No se encontró historial para las personas del período actual.")
        st.stop()

    # ACTUAL
    gb_curr_sum = df_curr.groupby(keys, as_index=False).agg({
        "PERIODO DE LECTURA":"last","COMPAÑÍA":"last","CÓDIGO DE DOSÍMETRO":"last","NOMBRE":"last","CÉDULA":"last",
        "FECHA_DE_LECTURA_DT":"max","TIPO DE DOSÍMETRO":"last","Hp10_NUM":"sum","Hp007_NUM":"sum","Hp3_NUM":"sum"})
    gb_curr_raw = df_curr.groupby(keys).agg({"Hp10_RAW":list,"Hp007_RAW":list,"Hp3_RAW":list}).rename(
        columns={"Hp10_RAW":"Hp10_ACTUAL_RAW_LIST","Hp007_RAW":"Hp007_ACTUAL_RAW_LIST","Hp3_RAW":"Hp3_ACTUAL_RAW_LIST"}).reset_index()
    out = gb_curr_sum.merge(gb_curr_raw, on=keys, how="left").rename(
        columns={"Hp10_NUM":"Hp10_ACTUAL_NUM_SUM","Hp007_NUM":"Hp007_ACTUAL_NUM_SUM","Hp3_NUM":"Hp3_ACTUAL_NUM_SUM"})
    ...
    FINAL_COLS = ["PERIODO DE LECTURA","COMPAÑÍA","CÓDIGO DE DOSÍMETRO","NOMBRE","CÉDULA",
