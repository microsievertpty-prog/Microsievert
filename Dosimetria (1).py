...
# IDs por defecto
DEFAULT_BASE_TABLE_ID   = "J"   # LISTA DE CÓDIGO (base)
...
# ===================== Normalización / Logo helpers =====================
def normalize_df(records):
    rows = []
    for r in records:
        f = r.get("fields", {}) or {}

        # soportar ambos nombres en REPORTE
        cod_rep = f.get("CÓDIGO DE DOSÍMETRO")
        if cod_rep is None:
            cod_rep = f.get("CÓDIGO_DOSÍMETRO")
...
def leer_dosis(upload):
    ...
    # normalizar nombres de columnas (baja sensibilidad a variantes)
...
# ===================== Adapta nombres de campo de código =====================
...
def construir_registros(dfp, dfd, periodo_filtro="— TODOS —"):
    """
    Genera TODOS los registros cruzando la tabla base (dfp) con el archivo de dosis (dfd).
    Soporta:
      A) esquema nuevo: CÓDIGO_DOSÍMETRO / PERIODO DE LECTURA (una fila por dosímetro)
      B) esquema viejo: DOSIMETRO 1..5 / PERIODO 1..5
    """
    registros = []
    ...
    if usa_nuevo:
        ...
        for need in ["NOMBRE","APELLIDO","CÉDULA","COMPAÑÍA","TIPO DE DOSÍMETRO","CÓDIGO USUARIO"]:
        ...
        for _, fila in dfp.iterrows():
            ...
            try:
                fecha_str = pd.to_datetime(fecha).strftime("%d/%m/%Y %H:%M") if pd.notna(fecha) else ""
            except Exception:
                fecha_str = ""
            ...
            registros.append({
                ...
                "CÓDIGO_DOSÍMETRO": cod,     # nuevo
                "CÓDIGO DE DOSÍMETRO": cod,  # alias para compatibilidad
...
def build_formatted_excel(df_final: pd.DataFrame,
    ...
    # (Sección informativa… igual que tu versión previa)
    row = last_row + 2
...
with tab1:
    ...
    # Mapas de columnas (acepta ambos nombres de código)
    CUSTOM_MAP = {
        "PERIODO DE LECTURA": "PERIODO DE LECTURA",
        "COMPAÑÍA": "COMPAÑÍA",
        "CÓDIGO DE DOSÍMETRO": "CÓDIGO DE DOSÍMETRO",  # viejo
        "CÓDIGO_DOSÍMETRO": "CÓDIGO_DOSÍMETRO",        # nuevo
        "NOMBRE": "NOMBRE",
        "CÉDULA": "CÉDULA",
