...
# ========= CONFIG NINOX (ajusta IDs si cambian) =========
...
# ========= VALOR - CONTROL (detección por contenido) =========
def aplicar_valor_menos_control(registros):
    ...
    if "ETIQUETA" in df.columns:
        control_mask |= df["ETIQUETA"].fillna("").str.strip().str.upper().eq("CONTROL")
    ...
    if not ctrl.empty:
        ...
        # fallback: si hay un código llamado CONTROL
        code_ctrl = df[df["CÓDIGO DE DOSÍMETRO"].astype(str).str.upper().eq("CONTROL")]
        if not code_ctrl.empty:
            base10 = float(pd.to_numeric(code_ctrl["Hp(10)"], errors="coerce").fillna(0).mean())
            base07 = float(pd.to_numeric(code_ctrl["Hp(0.07)"], errors="coerce").fillna(0).mean())
            base3  = float(pd.to_numeric(code_ctrl["Hp(3)"], errors="coerce").fillna(0).mean())
        else:
